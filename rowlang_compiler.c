...
typedef struct {
    TokenType type;
    int value; // For repetition count
} Token;

// Function to read the next token from the input source
Token nextToken(FILE *source) {
    ...
    // Read repetition count if available
...
// Function to emit C code for a given RowLang token
...
// Main function to compile RowLang source code
